...
import { LanguageDetection, LicenseAnalysis } from './types';
import { LANGUAGE_SAMPLE_LIMIT } from './detectors/languageDetector';
...
export function generateReport(
  ...
  return {
    ...
    scanDetails: {
      ...
      sampledLanguageFiles: Math.min(inventory.languageFiles.length, LANGUAGE_SAMPLE_LIMIT),
...
function renderScanDetails(details: Report['scanDetails']): string {
  const lines: string[] = [];
  lines.push('扫描详情：');
  lines.push(`- 扫描前端文件总数：${details.scannedFrontendFiles} 个`);
  lines.push(`- 字体文件：${details.scannedFontFiles} 个`);
  lines.push(`- 图片文件：${details.scannedImageFiles} 个`);
  lines.push(`- LICENSE 文件：${details.scannedLicenseFiles} 个`);
  lines.push(`- 语言采样文件：${details.sampledLanguageFiles} 个`);
  lines.push(
    `- 前端框架识别：${details.frontendFrameworkHints.length > 0 ? details.frontendFrameworkHints.join(', ') : '未识别'}`
  );
...
export function formatReport(report: Report, format: 'json' | 'html' | 'text'): string {
  if (format === 'json') {
...
<body class="bg-gray-50 min-h-screen">
  <div class="max-w-6xl mx-auto px-4 py-8">
    ...
    <header class="mb-8">
      ...
      <p class="text-gray-500 text-sm">${new Date(report.timestamp).toLocaleString('zh-CN')}</p>
    </header>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-red-200 p-6 ${report.summary.high > 0 ? 'ring-2 ring-red-100' : ''}">
        <div class="flex items-center justify-between mb-2">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-amber-200 p-6 ${report.summary.medium > 0 ? 'ring-2 ring-amber-100' : ''}">
        <div class="flex items-center justify-between mb-2">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
      <div class="flex items-center justify-between">
        <div>
          <h3 class="font-semibold text-gray-900">联网检测</h3>
          <p class="text-sm text-gray-500">${report.onlineDetection.enabled ? (report.onlineDetection.success ? '已启用并成功完成' : '已启用但失败') : '未启用（需在设置中开启）'}</p>
        </div>
        <span class="px-3 py-1 rounded-full text-sm font-medium ${report.onlineDetection.enabled ? (report.onlineDetection.success ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700') : 'bg-gray-100 text-gray-600'}">
          ${report.onlineDetection.enabled ? (report.onlineDetection.success ? '✓ 已启用' : '✗ 失败') : '○ 未启用'}
        </span>
      </div>
      ${report.onlineDetection.error ? `<p class="mt-2 text-sm text-red-600">错误: ${escapeHtml(report.onlineDetection.error)}</p>` : ''}
    </div>
  ...
  const lines: string[] = [];
  lines.push('IP Guard 扫描报告');
  lines.push(`时间: ${report.timestamp}`);
  lines.push(`项目: ${report.projectPath}`);
  lines.push(`扫描模式: ${report.scanMode}（设置来源: ${report.settingsSource}）`);
  lines.push(
    `语言目标市场: ${report.language.marketLabel} (${report.language.languageCode}, 置信度=${report.language.confidence.toFixed(2)})`
  );
  lines.push(`项目许可证: ${report.license.licenseName ?? '未检测到'} - ${report.license.message}`);
  lines.push(`联网检测: ${report.onlineDetection.enabled ? (report.onlineDetection.success ? '已启用' : '失败') : '未启用'}`);
  lines.push(`汇总: total=${report.summary.total}, high=${report.summary.high}, medium=${report.summary.medium}, low=${report.summary.low}`);
  lines.push('');
  lines.push(renderScanDetails(report.scanDetails));
  lines.push('');
  lines.push('检测结果:');
  ...
  for (const result of report.results) {
    lines.push(`- [${result.risk}] ${result.type} ${result.file ?? ''}`.trim());
